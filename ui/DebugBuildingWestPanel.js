/**
 * Debug section: offset x, y, width, height của West park (image).
 * Gắn vào container (modal body) khi có container.
 */
import { EventBus } from '../core/EventBus.js'
import { setDebugWestParkOverrides, clearDebugWestParkOverrides } from '../utils/DebugWestPark.js'

let spriteRef = null
let baseRef = null

function onBuildingWestSpriteCreated({ sprite, baseX, baseY, baseWidth, baseHeight }) {
  spriteRef = sprite
  baseRef = { baseX, baseY, baseWidth, baseHeight }
  const inputX = document.getElementById('debug-west-x')
  const inputY = document.getElementById('debug-west-y')
  const inputW = document.getElementById('debug-west-width')
  const inputH = document.getElementById('debug-west-height')
  if (inputX) {
...
function mountSection(container, syncFns) {
  ...
  section.innerHTML = `
    <div class="debug-modal-section-title">Park West (image)</div>
    <p class="debug-panel-hint">Căn chỉnh offset / size. Lưu tự động.</p>
    <div class="debug-panel-row">
      <label>X</label>
      <input type="number" id="debug-west-x" step="1" />
    </div>
    <div class="debug-panel-row">
      <label>Y</label>
      <input type="number" id="debug-west-y" step="1" />
    </div>
    <div class="debug-panel-row">
      <label>Width</label>
      <input type="number" id="debug-west-width" step="1" min="1" />
    </div>
    <div class="debug-panel-row">
      <label>Height</label>
      <input type="number" id="debug-west-height" step="1" min="1" />
    </div>
    <div class="debug-panel-actions">
      <button type="button" id="debug-west-reset">Reset (mặc định)</button>
    </div>
  `
  ...
  const inputW = document.getElementById('debug-west-width')
  const inputH = document.getElementById('debug-west-height')
  const resetBtn = document.getElementById('debug-west-reset')
...
export function initDebugBuildingWestPanel(container, syncFns) {
  if (!container) return
  mountSection(container, syncFns)
  EventBus.on('buildingWestSpriteCreated', onBuildingWestSpriteCreated)
}
